...
class evo_settings_settings{
		...
		return apply_filters('eventon_settings_tab1_arr_content', array(
			...
			array(
				...
				'fields'=> apply_filters('eventon_settings_time', array(
					array('id'=>'evcal_sh001','type'=>'subheader','name'=>__('General Time/Date Settings','eventon')),
						...
						array('id'=>'evo_utcoff','type'=>'yesno',
							'name'=>__('Use UTC offset time globally on calendars (BETA)','eventon'), 
			...
			array(
				...
				'fields'=>array(	
					array('id'=>'daig','type'=>'note',
						'name'=>'The below options are for testing and debuging eventon environment. They can provide general guidance for verification of proper functionality of EventON features.',
					),		
	...
	// single events
		function single_events(){
			...
			$data[] = array('id'=>'evosm_diencode','type'=>'yesno','name'=>'Disable social media event link encoding for special characters', 
				'legend'=>'Enabling this will stop encoding the URL of event on social share options including email share.');				
	...
	// html for diagnosis content
		function content_diag(){
			...
			<div class="diagnosis_row">
				...
				<p><?php _e('NOTE: The email send test use wordpress wp_mail() function. If you are having trouble sending emails via your website, we suggest using a SMTP plugin or using <a href="https://wordpress.org/plugins/wp-ses/">Amazon SES</a>.','eventon');?></p>
			</div>
			<?php
			return ob_get_clean();
		}
